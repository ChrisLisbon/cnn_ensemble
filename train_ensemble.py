...
def baseline_predict(dates_range):
    baseline_predictions_folder = 'matrices/kara_sea_meanyears_prediction'
    prediction = []
    for date in dates_range:
        matrix = np.load(f'{baseline_predictions_folder}/meanyears_kara_{date}.npy')
...
def create_train_dataset_for_ensemble(dates_range):
    ...
    cnn1.load_state_dict(torch.load('single_models/kara_104_52_l1.pt'))
    cnn2 = init_model(pre_history_size, forecast_size)
    cnn2.load_state_dict(torch.load('single_models/kara_104_52_ssim.pt'))
...
def train_ensemble(features, target, epochs, loss_name='l1'):
    ...
    torch.save(ensembling_cnn.state_dict(), f"ensemble_models/kara_52_{loss_name}.pt")
...
train_ensemble(features, target, epochs=500, loss_name='l1')